...
int interpolation_search(int arr[], int target, int n) {
	...
	while (low <= high && target >= arr[low] && target <= arr[high]) {
		...
		int interpol = low + (target - arr[low]) * (high - low) / (arr[high] - arr[low]);

		if (target == arr[interpol]) return interpol;

		else if (target < arr[interpol]) low = interpol + 1;

		else high = interpol - 1;
	}
